...
exports.generateSystemDocumentType = async function (req, res) {
  try {
    ...
    let salary_certificate = "64a7dbd84a9127eeea8e083e";
    ...
    const outputPathFolder = path.join(__dirname, path_name);
    const outputPath = path.join(__dirname, path_name, file_name);

    //Create the directory if it doesn't exist
    if(!fs.existsSync(outputPathFolder)) {
      fs.mkdirSync(outputPathFolder, {recursive : true});
    }
