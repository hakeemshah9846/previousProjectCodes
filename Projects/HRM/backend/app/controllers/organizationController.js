...
exports.enableExternalService = async function (req, res) {
  try {
    ...
    let disabled_flag;
    let reset_flag;
    //Checking if service already found or not
    const isFound = await organizations.find({
      $and: [
        {
          _id: organization_id,
    ...
    if (Number(isFound.length) > Number(0)) {
      for (let i = 0; i < isFound.length; i++) {
        ...
        let external_services_obj = objs.external_services;

        let target_service_ids = external_services_obj.map((e) => {
          let target_service_id = e.service_id;
          let target_status = e.status;
          if (target_status.toString() == "enabled") {
            return target_service_id.toString();
          } else if (target_status.toString() == "disabled") {
            disabled_flag = true;
          } else if (target_status.toString() == "reset") {
            reset_flag = true;
          }
        });
        if (target_service_ids.includes(service_id.toString())) {
          foundIndex = true;
        }
        if (foundIndex) {
          found_flag = true;
    ...
    let service_obj = {};
    let service_id_converted = new mongoose.Types.ObjectId(service_id);
    console.log("Disabled flag : ", disabled_flag);
    console.log("Reset flag : ", reset_flag);

    //Condition executes only if status disabled or reset, if found and status is enabled then already found error will happen
    if (!found_flag) {
      if (disabled_flag) {
        console.log("Disabled flag on, request to enable service...");
        //Saving service_obj, client_id, client_secret to database
        let update = await organizations.findOneAndUpdate(
          {
            $and: [
              { _id: organization_id },
              { deleted: { $ne: true } },
              { "external_services.service_id": service_id_converted },
            ],
          },
          { $set: { "external_services.$.status": "enabled" } },
          { new: true }
        );
        ...
        console.log("Reset flag on, request to reset datas...");

        //Saving service_obj, client_id, client_secret to database
        if (service_id.toString() === "64a3e8efaf6c697ec74d632b") {
          console.log("Request to reset zoho datas...");
          ...
          var updated_redirect_uri = `http://localhost:5000/auth/zoho/account/callback/${organization_id}`;
          var authorization_uri = `https://accounts.zoho.in/oauth/v2/auth?scope=ZohoMail.partner.organization.ALL&client_id=${client_id}&response_type=code&access_type=offline&prompt=consent&redirect_uri=http://localhost:5000/auth/zoho/account/callback/${organization_id}`;
          ...
          let update = await organizations.findOneAndUpdate(
            ...
            {
              $set: {
                "external_services.$.status": "enabled",
                zoho_client_id: client_id,
                zoho_client_secret: client_secret,
                zoho_redirect_uri : updated_redirect_uri,
              },
            },
            { new: true }
          );
          ...
          let response = error_function({ status: 400, message: "This service is currently not available" });
          res.status(response.statusCode).send(response);
          return;
        }
      } else {
        if (service_id.toString() === "64a3e8efaf6c697ec74d632b") {
          ...
          let update = await organizations.findOneAndUpdate(
            { $and: [{ _id: organization_id }, { deleted: { $ne: true } }] },
            // {$set : {$push : {external_services : service_obj}, zoho_client_id : client_id, zoho_client_secret : client_secret}},
            {
              $push: { external_services: service_obj },
              zoho_client_id: client_id,
              zoho_client_secret: client_secret,
      ...
      let response = error_function({ status: 409, message: "Already found" });
      res.status(response.statusCode).send(response);
      return;
    }
  } catch (error) {
...
exports.disableExternalService = async function (req, res) {
  try {
    ...
    let update = await organizations.findOneAndUpdate(
      ...
      {
        $set: {
          "external_services.$.status": "disabled",
        },
      },
      { new: true }
    );
...
exports.resetExternalService = async function (req, res) {
  try {
    const authHeader = req.headers["authorization"];
    let token = authHeader ? authHeader.split(" ")[1] : null;
    let decoded = jwt.decode(token);
    ...
    let update = await organizations.findOneAndUpdate(
      ...
      {
        $set: {
          "external_services.$.status": "reset",
        },
      },
      { new: true }
    );
...
exports.zohoCallBack = async function (req, res) {
  try {
    ...
    // let client_id = organization.zoho_client_id;
    // let client_secret = organization.zoho_client_secret;
    // let redirect_uri = organization.zoho_redirect_uri;
    ...
    // const accessToken = zoid_response.data.access_token;
    // const refreshToken = zoid_response.data.refresh_token;

    // console.log("Zoid AccessToken : ", accessToken);
    // console.log(" ZoidRefreshToken : ", refreshToken);
    ...
    if (zoid && refreshToken) {
      ...
      let save_zoid = await organizations.updateOne(
        ...
        { zoho_zoid: zoid, zoho_refresh_token : refreshToken  },
        { new: true }
      );
